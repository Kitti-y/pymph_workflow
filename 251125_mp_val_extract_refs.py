...
for files in os.listdir('run_ref'):
    if files.endswith('soid.mph'):
        filepath = os.path.join('run_ref', files)
        ...
        modeljava.result().numerical("int1").selection().named("sel3")
        modeljava.result().numerical("int1").setIndex("expr", "V", 0)
        modeljava.result().numerical("int1").run()
        result = modeljava.result().numerical("int1").getReal()
        client.clear()

        array_result = np.array(result)
        flatten_arr = array_result.flatten()
...
#df_results.to_csv('251120_mp_extracted_results.csv', index=False)
df_results.to_csv('251120_mp_extracted_resultsref_sph.csv', index=False)
